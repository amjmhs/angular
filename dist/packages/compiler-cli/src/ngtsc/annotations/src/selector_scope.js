...
class SelectorScopeRegistry {
     ...
     */
    registerSelector(node, selector) {
        node = ts.getOriginalNode(node);
    ...
    lookupScopes(node, ngModuleImportedFrom) {
        ...
        return {
            compilation: [
                ...
                ...flatten(data.imports.map(ref => this.lookupScopes(ref.node, absoluteModuleName(ref)).exported)),
                // And include the compilation scope of exported modules.
                ...flatten(data.exports.filter(ref => this._moduleToData.has(ref.node))
                    .map(ref => this.lookupScopes(ref.node, absoluteModuleName(ref))
                    .exported))
            ],
            exported: flatten(data.exports.map(ref => {
                if (this._moduleToData.has(ref.node)) {
                    return this.lookupScopes(ref.node, absoluteModuleName(ref)).exported;
                }
                else {
                    return [ref];
                }
     ...
     */
    _readMetadataFromCompiledClass(clazz, ngModuleImportedFrom) {
     ...
     * ngDirectiveDef.
     */
    _readSelectorFromCompiledClass(clazz) {
    ...
    _extractReferencesFromType(def, ngModuleImportedFrom) {
        ...
        return def.elementTypes.map(element => {
            ...
            const clazz = node;
            const id = reflector_1.reflectIdentifierOfDeclaration(clazz);
...
function convertReferenceMap(map, context) {
    return new Map(Array.from(map.entries()).map(([selector, ref]) => [selector, util_1.referenceToExpression(ref, context)]));
}
