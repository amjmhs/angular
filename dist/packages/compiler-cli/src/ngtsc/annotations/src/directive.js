...
class DirectiveDecoratorHandler {
    ...
    compile(node, analysis) {
        ...
        const res = compiler_1.compileDirectiveFromMetadata(analysis, pool, compiler_1.makeBindingParser());
        return {
            name: 'ngDirectiveDef',
...
function extractDirectiveMetadata(clazz, decorator, checker, reflector, isCore) {
    ...
    const inputsFromFields = parseDecoratedFields(metadata_1.filterToMembersWithDecorator(decoratedElements, 'Input', coreModule), reflector, checker);
    // And outputs.
    const outputsFromMeta = parseFieldToPropertyMapping(directive, 'outputs', reflector, checker);
    const outputsFromFields = parseDecoratedFields(metadata_1.filterToMembersWithDecorator(decoratedElements, 'Output', coreModule), reflector, checker);
    ...
    if (directive.has('selector')) {
        const resolved = metadata_1.staticallyResolve(directive.get('selector'), reflector, checker);
...
function extractQueryMetadata(name, args, propertyName, reflector, checker) {
    ...
    const arg = metadata_1.staticallyResolve(args[0], reflector, checker);
...
function extractQueriesFromDecorator(queryData, reflector, checker, isCore) {
    ...
    metadata_1.reflectObjectLiteral(queryData).forEach((queryExpr, propertyName) => {
        ...
        const query = extractQueryMetadata(type.name, queryExpr.arguments || [], propertyName, reflector, checker);
...
function parseFieldToPropertyMapping(directive, field, reflector, checker) {
    ...
    const metaValues = metadata_1.staticallyResolve(directive.get(field), reflector, checker);
...
function extractHostBindings(metadata, members, reflector, checker, coreModule) {
    ...
    if (metadata.has('host')) {
        const hostMetaMap = metadata_1.staticallyResolve(metadata.get('host'), reflector, checker);
