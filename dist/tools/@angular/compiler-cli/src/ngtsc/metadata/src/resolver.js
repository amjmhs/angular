 ...
 * referenceable.
 */
class NodeReference extends Reference {
    constructor(node, moduleName) {
        super(node);
        this.moduleName = moduleName;
    }
    toExpression(context) { return null; }
    withIdentifier(identifier) { return this; }
}
 ...
 */
class ResolvedReference extends Reference {
    constructor(node, identifier) {
        super(node);
        this.identifier = identifier;
        this.expressable = true;
    }
    toExpression(context) {
        if (ts.getOriginalNode(context) === ts.getOriginalNode(this.identifier).getSourceFile()) {
            ...
            let relative = path.posix.relative(path.dirname(context.fileName), this.node.getSourceFile().fileName)
...
class AbsoluteReference extends Reference {
    constructor(node, identifier, moduleName, symbolName) {
        super(node);
        this.identifier = identifier;
        this.moduleName = moduleName;
...
function staticallyResolve(node, host, checker, foreignFunctionResolver) {
    return new StaticInterpreter(host, checker).visit(node, {
        ...
        scope: new Map(), foreignFunctionResolver,
    });
}
...
class StaticInterpreter {
    ...
    visitIdentifier(node, context) {
        ...
        const result = this.visitDeclaration(decl.node, Object.assign({}, context, { absoluteModuleName: decl.viaModule || context.absoluteModuleName }));
    ...
    visitSourceFile(node, context) {
        ...
        declarations.forEach((decl, name) => {
            const value = this.visitDeclaration(decl.node, Object.assign({}, context, { absoluteModuleName: decl.viaModule || context.absoluteModuleName }));
            map.set(name, value);
        });
        return map;
    }
    accessHelper(lhs, rhs, context) {
        const strIndex = `${rhs}`;
    ...
    visitCallExpression(node, context) {
        ...
        if (fn.body === undefined) {
            ...
            return this.visitExpression(expr, Object.assign({}, context, { absoluteModuleName }));
        }
        const body = fn.body;
        ...
        return ret.expression !== undefined ?
            this.visitExpression(ret.expression, Object.assign({}, context, { scope: newScope })) :
            undefined;
    }
