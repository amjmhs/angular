...
var TscWatch = /** @class */ (function () {
    ...
    TscWatch.prototype.watch = function () {
        var _this = this;
        var tsc = Promise
            .all(this.tsconfig.map(function (tsconfig) {
            var args = [exports.TSC, '--emitDecoratorMetadata', '--project', tsconfig];
            if (!_this.runOnce)
                args.push('--watch');
            return _this.runCmd(args, {}, function (d) { return _this.consumeLine(d, false); }, function (d) { return _this.consumeLine(d, true); });
        }))
            .then(function (exitCodes) {
            return exitCodes.reduce(function (prevValue, currValue) { return Math.max(prevValue, currValue); }, 0);
        });
        if (this.runOnce) {
    ...
    TscWatch.prototype.runCmd = function (argsOrCmd, env, stdOut, stdErr) {
        ...
        if (typeof argsOrCmd == 'function') {
            ...
            var args_1 = argsOrCmd;
            return new Promise(function (resolve, reject) {
                var cmd = args_1[0], options = args_1.slice(1);
                ...
                childProcess.on('close', function (code) {
                    ...
                    console.log('EXIT:', code, '<=====', args_1.join(' '));
    ...
    TscWatch.prototype.consumeLine = function (buffer, isStdError) {
        ...
        if (contains(line, this.start)) {
            ...
            if (this.state == State.error) {
                ...
                if (this.triggered) {
                    this.triggered.then(function () { return _this.triggerCmds(); }, function (e) { return console.log('Error while running commands....', e); });
                }
                else {
                    this.triggerCmds();
                }
    ...
    TscWatch.prototype.triggerCmds = function () {
        ...
        this.onChangeCmds.forEach(function (cmd) { return cmdPromise =
            cmdPromise.then(function () { return _this.runCmd(cmd); }); });
